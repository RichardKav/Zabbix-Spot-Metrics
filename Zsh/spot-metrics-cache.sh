...
while true;
do 
  TOPARSE=$(perf stat -a -x, --log-fd 1 -e cache-references,cache-misses,page-faults,LLC-loads,LLC-stores sleep $SLEEP_TIME 2>&1)
  TOPARSE=$(echo $TOPARSE | tr ',' ' ' | tr '\n' ' ')
  #read -a PARSED <<< $(echo $TOPARSE | tr ',' ' ')
  PARSED=("${(@s/ /)TOPARSE}")
  unset TOPARSE
  CACHEREF=${PARSED[0]//[[:blank:]]/}
  CACHEMISS=${PARSED[2]//[[:blank:]]/}
  STR="scale = 10;$CACHEMISS/$CACHEREF" 
  CACHEMISSFRACTION="$( echo $STR | bc )"
  echo CACHEMISSFRACTION $CACHEMISSFRACTION
